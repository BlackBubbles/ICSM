...
Version: 1.0.0
Description:
    The following python file contains the code that adds the quick 
    access drop down menu for the TDI
'''
...
'''
The following function shows the TDI drop down menus to the user based on
where the user clicked on the notebook
'''
def showMenuDropDown(event, config, QAMenu, TDIMenu):
  
  # Check to make sure that only the quick access tab is being clicked
  # on
  if event.widget.identify(event.x, event.y) == 'label' or\
    ...
    # If the quick access tab is not being clicked on then post the
    # default menu for the TDI
    else:
