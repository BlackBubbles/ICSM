...
Version: 0.0.1
Description:
    The following python file contains the code that tests the Controller
    to make sure that nothing is initially wrong
'''
...
"configActions.py" file
'''
def __testConfig(testF, config):
  return True, ""

'''
Initial test function for the Controller that calls all the private
  ...
  if isinstance(config, ModuleType):
    if hasattr(config, 'confirm'):
      if not config.confirm('Actions'):
        return False, "%s:\nconfig file for Actions is not configActions"\
                      % ERROR
    else:
